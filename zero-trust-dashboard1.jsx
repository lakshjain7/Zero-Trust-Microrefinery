...
// ── STATE CONSTANTS ──
const STATES = { IDLE: "IDLE", EXECUTING: "EXECUTING", PAUSED: "PAUSED", BREACH: "BREACH DETECTED", ERROR: "ERROR" };
const STATE_COLORS = { [STATES.IDLE]: "#6b7280", [STATES.EXECUTING]: "#00FF9D", [STATES.PAUSED]: "#FFD166", [STATES.BREACH]: "#FF2A2A", [STATES.ERROR]: "#FF6B35" };
...
export default function Dashboard() {
  ...
  // ── HEARTBEAT ──
  useEffect(() => {
    const t = setInterval(() => { heartbeatRef.current = (heartbeatRef.current + 1) % 2; }, 2000);
  ...
  useEffect(() => {
    if (systemState === STATES.BREACH) {
      setBreachFlash(3);
      const beats = [0, 300, 600];
      beats.forEach((d, i) => setTimeout(() => setBreachFlash(3 - i), d));
  ...
  const addMqtt = useCallback((topic, status, hash) => {
    ...
    setMqttLog(l => [...l.slice(-20), { ts, topic, status, hash }]);
    if (status === "VERIFIED") setPacketsVerified(p => p + 1);
    else setPacketsDropped(p => p + 1);
  }, []);

  const addAi = useCallback((text, type = "reason") => {
    setAiLog(l => [...l.slice(-15), { text, type, id: Date.now(), active: true }]);
  ...
  const runTestA = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      addMqtt("cmd/pump", "VERIFIED", "b7e2...d45a");
    }, 5900);
    setTimeout(() => {
    ...
    setTimeout(() => {
      setActivePumps({ red: false, blue: false, yellow: false });
      addMqtt("cmd/pump", "VERIFIED", "e4a1...b83c");
  ...
  return (
    <div style={{
      width: "100%", minHeight: "100vh", background: "radial-gradient(ellipse at 30% 20%, #12121A 0%, #0B0C10 70%)",
      ...
      {/* ── AMBIENT BG ── */}
      <div style={{
        position: "fixed", inset: 0, background: "radial-gradient(circle at 20% 80%, #9D4EDD08 0%, transparent 50%), radial-gradient(circle at 80% 20%, #45F3FF06 0%, transparent 50%)",
      ...
      {sweepDir && (
        <div style={{
          position: "fixed", inset: 0, zIndex: 100, pointerEvents: "none", overflow: "hidden"
        }}>
          <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: isBreaching ? "#FF2A2A22" : `${stateColor}15`,
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", fontSize: 11, fontFamily: "'JetBrains Mono', monospace" }}>
            <PulseDot color={stateColor} /> SYSTEM: {systemState === STATES.EXECUTING ? "ACTIVE" : systemState === STATES.IDLE ? "IDLE" : systemState === STATES.PAUSED ? "PAUSED" : "ALERT"}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "30% 40% 30%", gap: 16, padding: 16, height: "calc(100vh - 56px)", position: "relative", zIndex: 5 }}>
          ...
          <GlassPanel accent="#45F3FF" focused={focusPanel === "left"} dimmed={focusPanel && focusPanel !== "left"} style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 12, marginTop: 8 }}>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                {["Mix 50ml Purple / ≤300mA", "2x Green / Cup Swap", "Run Attack Demo"].map((cmd, i) => (
                  <button key={i} onClick={() => i === 0 ? runTestA() : i === 1 ? runTestB() : runTestC()}
                    style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
          ...
          <GlassPanel accent="#3B82F6" focused={focusPanel === "center"} dimmed={focusPanel && focusPanel !== "center"} style={{ flex: 1, position: "relative", minHeight: 0 }}>
            ...
            <svg viewBox="0 0 500 280" style={{ width: "100%", marginTop: 8 }}>
              ...
              {[{ color: "#FF2A2A", label: "RED", y: 30, pump: "red", rate: "8.5ml/s" },
                ...
                <g key={i}>
                  ...
                  {/* Tube */}
                  <path d={`M 70 ${cup.y + 27} C 150 ${cup.y + 27}, 200 150, 300 150`} fill="none" stroke={activePumps[cup.pump] ? cup.color : "#333"} strokeWidth={activePumps[cup.pump] ? 3 : 2} opacity={activePumps[cup.pump] ? 0.8 : 0.3} strokeLinecap="round" />

                  {/* Flow particles */}
                  {activePumps[cup.pump] && [0, 1, 2, 3].map(p => (
                    <circle key={p} r={3} fill={cup.color} opacity={0.9}>
                      <animateMotion dur={`${1.2 + p * 0.15}s`} begin={`${p * 0.3}s`} repeatCount="indefinite" path={`M 70 ${cup.y + 27} C 150 ${cup.y + 27}, 200 150, 300 150`} />
                    </circle>
                  ))}

                  {/* Relay indicator */}
                  <rect x={100} y={cup.y + 20} width={14} height={14} rx={2} fill={activePumps[cup.pump] ? cup.color + "33" : "#1a1a2e"} stroke={activePumps[cup.pump] ? cup.color : "#333"} strokeWidth={1} />
                  {activePumps[cup.pump] && <circle cx={107} cy={cup.y + 27} r={3} fill={cup.color}>
              ...
              {cupFill > 5 && <path d={`M 313 ${90 + 90 - cupFill / 100 * 80} Q 330 ${87 + 90 - cupFill / 100 * 80} 345 ${90 + 90 - cupFill / 100 * 80} Q 360 ${93 + 90 - cupFill / 100 * 80} 377 ${90 + 90 - cupFill / 100 * 80}`} fill="none" stroke={cupColor} strokeWidth={1} opacity={0.6}>
                <animate attributeName="d" values={`M 313 ${90 + 90 - cupFill / 100 * 80} Q 330 ${87 + 90 - cupFill / 100 * 80} 345 ${90 + 90 - cupFill / 100 * 80} Q 360 ${93 + 90 - cupFill / 100 * 80} 377 ${90 + 90 - cupFill / 100 * 80};M 313 ${90 + 90 - cupFill / 100 * 80} Q 330 ${93 + 90 - cupFill / 100 * 80} 345 ${90 + 90 - cupFill / 100 * 80} Q 360 ${87 + 90 - cupFill / 100 * 80} 377 ${90 + 90 - cupFill / 100 * 80};M 313 ${90 + 90 - cupFill / 100 * 80} Q 330 ${87 + 90 - cupFill / 100 * 80} 345 ${90 + 90 - cupFill / 100 * 80} Q 360 ${93 + 90 - cupFill / 100 * 80} 377 ${90 + 90 - cupFill / 100 * 80}`} dur="2s" repeatCount="indefinite" />
              </path>}
              ...
              {/* HC-SR04 Sensor */}
              <rect x={330} y={60} width={30} height={18} rx={3} fill="#1a1a2e" stroke="#45F3FF" strokeWidth={1} opacity={0.7} />
              <text x={345} y={72} textAnchor="middle" fill="#45F3FF" fontSize={6} fontFamily="JetBrains Mono">SR04</text>
              {/* Scan line */}
              <line x1={345} y1={80} x2={345} y2={90} stroke={distance > 15 ? "#FF2A2A" : distance < 5 ? "#FF2A2A" : "#45F3FF"} strokeWidth={1} opacity={0.6}>
              ...
              <text x={345} y={55} textAnchor="middle" fill="#45F3FF" fontSize={9} fontFamily="JetBrains Mono" fontWeight={500}>{distance}cm</text>
            ...
            {/* Breach overlay */}
            {isBreaching && <div style={{ position: "absolute", inset: 0, background: "rgba(255,42,42,0.05)", animation: "glitch 0.3s ease-in-out 3", borderRadius: 12, pointerEvents: "none" }} />}
          </GlassPanel>

          {/* Action Bar */}
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => { if (systemState === STATES.IDLE) { setSystemState(STATES.EXECUTING); triggerSweep("ltr", "#45F3FF"); } }}
              disabled={systemState !== STATES.IDLE}
              style={{
                flex: 1, padding: "12px 0", borderRadius: 10, border: "none", cursor: systemState === STATES.IDLE ? "pointer" : "default",
                background: systemState === STATES.IDLE ? "linear-gradient(135deg, #45F3FF, #3B82F6)" : "#1a1a2e",
                color: systemState === STATES.IDLE ? "#000" : "#555", fontFamily: "'Space Grotesk', sans-serif",
                fontWeight: 700, fontSize: 13, letterSpacing: 1, transition: "all 0.2s"
              }}>
              {systemState === STATES.EXECUTING ? "⏳ RUNNING..." : "▶ START EXECUTION"}
            </button>
            <button onClick={() => { if (systemState === STATES.EXECUTING) { setSystemState(STATES.PAUSED); triggerSweep("rtl", "#FFD166"); } }}
              style={{
            ...
            <button onClick={() => { setSystemState(STATES.IDLE); setActivePumps({ red: false, blue: false, yellow: false }); setFocusPanel(null); }}
              style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
          <GlassPanel accent="#9D4EDD" focused={focusPanel === "right"} dimmed={focusPanel && focusPanel !== "right"} style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, position: "relative" }}>
            {isBreaching && <div style={{ position: "absolute", inset: 0, animation: "glitch 0.2s ease-in-out 5", borderRadius: 12, pointerEvents: "none", background: "rgba(255,42,42,0.03)" }} />}
            ...
            <div style={{ flex: 1, overflow: "auto", minHeight: 0, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, lineHeight: 1.8 }}>
              {mqttLog.length === 0 && <div style={{ color: "#444", fontSize: 11, padding: 16, textAlign: "center" }}>No packets intercepted</div>}
              {mqttLog.map((pkt, i) => {
                const isVerified = pkt.status === "VERIFIED";
                const isFail = pkt.status === "HMAC_FAIL";
                return (
                  <div key={i} style={{
                    padding: "4px 8px", borderRadius: 4, marginBottom: 3,
                    borderLeft: `2px solid ${isVerified ? "#00FF9D" : "#FF2A2A"}`,
                    background: isFail ? "rgba(255,42,42,0.1)" : "transparent",
                    animation: "fadeSlideUp 0.2s ease-out",
                    display: "flex", gap: 8, alignItems: "center", color: isVerified ? "#aaa" : "#FF2A2A"
                  }}>
                    <span style={{ color: "#555" }}>{pkt.ts}</span>
                    <span style={{ color: "#666" }}>{pkt.topic}</span>
                    <span style={{ color: isVerified ? "#00FF9D" : "#FF2A2A", fontWeight: 600 }}>{isVerified ? "✓" : "✗"} {pkt.status}</span>
                    <span style={{ color: "#555", marginLeft: "auto" }}>{pkt.hash}</span>
                  </div>
                );
              })}
            </div>
