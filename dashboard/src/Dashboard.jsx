    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{
            marginLeft: 12, display: "flex", alignItems: "center", gap: 6, padding: "4px 10px", borderRadius: 20,
            background: "rgba(0,255,157,0.05)", border: "1px solid rgba(0,255,157,0.2)",
            fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: "#00FF9D"
          }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, minHeight: 0 }}>
          <GlassPanel accent="#45F3FF" focused={focusPanel === "left"} dimmed={focusPanel && focusPanel !== "left"} style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 12, marginTop: 8 }}>
              ...
              {/* Real command presets — no more mocks! */}
              <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                {[
                  { label: "Mix 50ml Purple / ≤300mA", cmd: "Mix 50ml Purple with a 300mA current limit" },
                  { label: "Pour 20ml Red", cmd: "Actuate red pump for 20ml" },
                  { label: "Safety Test: 2x Green", cmd: "Mix 2 batches of Green (Blue + Yellow) sequentially" }
                ].map((item, i) => (
                  <button key={i} onClick={() => { setInputVal(item.cmd); sendCommand(item.cmd); }}
                    style={{
              ...
              {/* ── DIRECT OVERRIDE BUTTONS (New) ── */}
              <div style={{ display: "flex", gap: 6, marginTop: 12, alignItems: "center" }}>
                <span style={{ fontSize: 9, color: "#666", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, letterSpacing: 1 }}>DIRECT BYPASS:</span>
                {[
                  { label: "PUMP RED (2s)", cmd: "direct:red:2000" },
                  { label: "PUMP BLUE (2s)", cmd: "direct:blue:2000" },
                  { label: "PUMP YLW (2s)", cmd: "direct:yellow:2000" }
                ].map((item, i) => (
                  <button key={i} onClick={() => sendCommand(item.cmd)}
                    style={{
                      padding: "4px 8px", borderRadius: 6, border: "1px solid rgba(255,42,42,0.2)",
                      background: "rgba(255,42,42,0.05)", color: "#FF2A2A", fontSize: 9, cursor: "pointer",
                      fontFamily: "'JetBrains Mono', monospace", transition: "all 0.2s", fontWeight: 600
                    }}
                    onMouseEnter={e => { e.target.style.background = "rgba(255,42,42,0.15)"; e.target.style.borderColor = "#FF2A2A"; }}
                    onMouseLeave={e => { e.target.style.background = "rgba(255,42,42,0.05)"; e.target.style.borderColor = "rgba(255,42,42,0.2)"; }}
                  >{item.label}</button>
                ))}
              </div>
            </div>
